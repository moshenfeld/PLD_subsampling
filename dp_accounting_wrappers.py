...
def create_pld_and_extract_pmf(
    ...
    Args:
        standard_deviation: Gaussian mechanism sigma.
        sensitivity: L2 sensitivity (typically 1.0).
        sampling_prob: Sampling probability q in (0, 1]. If 1.0, returns the non-amplified PMF.
        value_discretization_interval: Discretization step used by dp-accounting.
        remove_direction: If True, return the remove-direction PMF; otherwise the add-direction PMF.

    Returns:
        A dp-accounting PLD PMF object (direction-specific), which exposes internal fields
        `_pmf_remove` and `_pmf_add`. This function returns one of those internal PMFs directly.
    """
...
def amplify_pld_separate_directions(
    ...
    Given a base (non-amplified) PLD, compute new PMFs corresponding to subsampling
    with probability `sampling_prob` for both directions, and return them as
    dp-accounting PLDPmf objects in a dict.

    Args:
        base_pld: dp-accounting PrivacyLossDistribution object (with `_pmf_remove`/`_pmf_add`).
        sampling_prob: Subsampling probability q in (0, 1]. If 1.0, returns the base PMFs.

    Returns:
        Dict with keys `pmf_remove` and `pmf_add`, each a PLDPmf object on the same discretization.
    """
    if not (0.0 < sampling_prob <= 1.0):
        raise ValueError("sampling_prob must be in (0, 1]")

    # Trivial case: no amplification
    if sampling_prob == 1.0:
        return {"pmf_remove": base_pld._pmf_remove, "pmf_add": base_pld._pmf_add}

    # Use the base remove-direction PMF as the reference grid and probabilities
    base_losses, base_probs = dp_accounting_pmf_to_loss_probs(base_pld._pmf_remove)

    # Compute amplified PMFs for each direction on the same grid
...
def dp_accounting_pmf_to_loss_probs(pld_pmf: Union[SparsePLDPmf, DensePLDPmf, Any]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns only finite values and normalizes to the finite target mass (1 - infinity_mass).
    """
